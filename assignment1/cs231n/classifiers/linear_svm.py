...
def svm_loss_naive(W, X, y, reg):
  ...
  for i in xrange(num_train):
    ...
    for j in xrange(num_classes):
      ...
      if margin > 0:
        condition_not_met += 1
        # contribution of gradient of loss associated with example X[i] to an incorrect class's "W"
        dW[:,j] += X[i]
        loss += margin
    # contribution of gradient of loss associated with example X[i] to the correct class's "W"
    dW[:,y[i]] += -condition_not_met * X[i]
